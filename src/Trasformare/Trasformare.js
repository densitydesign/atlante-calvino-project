...
import filtroVirgi from "./filtroVirgi2022";
...
class Trasformare extends Component {
		...
		d3.tsv(process.env.PUBLIC_URL + "/places-matrix-data.tsv")
			...
			.then((data) => {
				...
				let serachByVolume = _volumes.map((v) => {
					...
					let texts = textsIds.map((id) =>
						searchByCompositionTitle.find((c) => id === c.sourceID)
					);
    ...
    toPreserve =  filtroVirgi.map(d=>d.id).flat();
		...
		return (
			<div className="trasformare main">
				...
				<div className="the-body-viz">
					...
					{!this.state.isLoading && (
						<PlacesMatrix
							...
							searchedPlace={
								this.state.cerca_per.options.find((d) => d.status).value ===
								"luogo"
							}
						/>
					)}
				...
				<div className="bottom-nav navigations">
					...
					{this.state.isLoading && (
						<Loading style={{ gridColumn: "span 10" }} />
					)}
					{!this.state.isLoading && (
						<RangeFilterSnap
