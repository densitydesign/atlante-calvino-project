...
let width,
  ...
  fontSize = 12,
  strokeWidth = 0.5,
  annotationsFontSize = 12,
  annotationsStrokeWidth = 1,
  zoom,
  svg,
  g0,
  g1,
...
V.init = async (options) => {
  ...
  zoom = d3
    ...
    .on("zoom", () => {
      ...
      g3.selectAll("path,line").attr(
        "stroke-width",
        annotationsStrokeWidth / d3.event.transform.k
      );
      if (d3.event.transform.k >= 1.75) {
        label
          .attr("display", "block")
          .attr("font-size", fontSize / d3.event.transform.k);
  ...
  gStyles = g2.append("g").classed("gStyles", true);
  g3 = g1
    .append("g")
    .classed("g3", true)
    .attr("display", "none")
  ...
  const annotations = d3
    .select(incomingSVG)
    .select("#annotazioni")
    .attr("transform", "translate(" + -2740 / 2 + "," + -1568 / 2 + ")");
  annotations
    .selectAll("#lines path, #lines line")
...
V.update = (options) => {
  ...
  sRiform = sRiform.data(reducedData);
  sRiform.exit().remove();
  sRiform = sRiform
    .enter()
    .append("circle")
  ...
  length
    .filter((d) => d.perc_dubbio <= minPerc)
    .attr("fill", "#ddddda")
    .attr("fill-opacity", "1")
    .attr("stroke", "white");
  ...
  g1.attr(
    "transform",
    `translate(-102, 48) scale(1.05) rotate(-9.4 ${width / 2} ${height / 2})`
  );
};
...
V.changeColor = (value) => {
  if (
    ...
    combinations.select("path").attr("fill", (d) => {
    ...
    combinations.select("path").attr("fill", (d) => d.color);
  }
};
...
function interpolateColor(d) {
  const tot_cosa = Number(
    d["cosa_negazione"] +
      Number(d["cosa_esitazione"]) +
      Number(d["cosa_riformulazione"])
  );
  const tot_come = Number(
    d["come_negazione"] +
      Number(d["come_esitazione"]) +
      Number(d["come_riformulazione"])
  );
  const tot_senso = Number(
    d["senso_negazione"] +
      Number(d["senso_esitazione"]) +
      Number(d["senso_riformulazione"])
  );
  const col_r =
    tot_cosa * d3.color(col_macrocategorie("cosa")).r +
    tot_come * d3.color(col_macrocategorie("come")).r +
    tot_senso * d3.color(col_macrocategorie("senso")).r;
  const col_g =
    tot_cosa * d3.color(col_macrocategorie("cosa")).g +
    tot_come * d3.color(col_macrocategorie("come")).g +
    tot_senso * d3.color(col_macrocategorie("senso")).g;
  const col_b =
    tot_cosa * d3.color(col_macrocategorie("cosa")).b +
