...
function Worm({ racconto, circles, radius, labelSize, wormSize }) {
  const wormStart = radius - labelSize - wormSize;
  const wormEnd =  radius - labelSize

  const circleRadius = wormSize / circles.length / 2
  
  const yScale = scaleLinear().domain([0, 2]).range([0, circleRadius * 2])

  return (
    <g>
      
      {circles.map((circle, i) => {
        return <g key={i}><circle 
          className="movement"
          style={{fill: colorScale(circle.category)}}
          r={circleRadius * 1.5} 
          cy={yScale(circle.level)}
          cx={wormStart + (i * circleRadius * 2) + circleRadius}></circle>
          {circle.place && <circle 
...
export default function CircleWorms({ size, circlesMap, racconti }) {
