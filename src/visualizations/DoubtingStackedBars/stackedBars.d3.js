...
V.initialize = (el, data_for_update, _onSelectedElement, t) => {
  ...
  xAxis = svg
    .append("g")
    .classed("axis x-axis stacked-bars-dubbio", true)
...
V.update = (data, stackMode, baseLayer) => {
  ...
  const updateLegend = () => {
    if (svg.selectAll(".selected").size() === 0) {
      legend
        .transition()
        .duration(350)
        .attr("transform", `translate(${margin.left + 21},${5})`)
    } else {
      legend
        .transition()
        .duration(350)
        .attr("transform", `translate(${margin.left / 2 + 13},${5})`)
    }
    ...
    legendItem = legendItem
      ...
      .attr("transform", (d, i) => {
        ...
        if (d.id.includes("definitivo")) {
          ...
          let sss = legendData
            .filter((item) => item.id.includes("soggetto"))
            .map((item) => item.id)
            .indexOf(d.id)
          sss++
      ...
      .html((d) => {
        let this_percentage = ""
        if (showPercentage && d.percentage)
          this_percentage =
            " " +
            (d.percentage < 1
  ...
  const removeSelectionAll = () => {
    d3.select(".bottom-nav.navigations")
      .style("opacity", 1)
    ...
    leaf_misto.exit().transition().duration(350).style("opacity", 0).remove()

    leaf_soggetto = leaf_soggetto.data([])
    leaf_soggetto.exit().transition().duration(350).style("opacity", 0).remove()
  ...
  serie_rect = serie_rect
    ...
    .on("click", function (d) {
      if (
        ...
        !d3.select(this).classed("filtered")
      ) {
        selection(d, d3.select(this).classed("selected"))
      } else {
        if (performed_selection_data) {
          performed_selection_data = null
          d3.selectAll(".x-axis.stacked-bars-dubbio .tick").style(
            "display",
            "none"
          )
          removeSelectionAll()
  ...
  const preSelection = (d) => {
    const bar = d3
      ...
      .filter((rect) => rect.data.id === d.data.id)
    bar.style("opacity", 1)
    let selected_id = performed_selection_data
      ? performed_selection_data.data.id
      : "nessuno"
    xAxis
      .selectAll(".tick")
      .filter((tick) => tick === d.data.id || tick === selected_id)
      .style("display", "block")
  }
  ...
  const selection = (d, isSelected) => {
    ...
    bar
      ...
      .attr(
        "transform",
        `translate(${-width_treemap / 2 + x.bandwidth() / 2},0)`
      )
    ...
    ticks_on_left
      ...
      .attr(
        "transform",
        (d) => `translate(${x(d) + x.bandwidth() / 2 - width_treemap / 2}, 0)`
      )
    ...
    if (data_misto) {
      ...
      d3.select(".treemap-misto").attr(
        "transform",
        `translate(${x_misto + margin.left}, ${y_misto + margin.top})`
      )

      legendData.find((d) => d.id === "definitivo").percentage =
        (d.data.definitivo / d.data.length) * 100
      legendData.find((d) => d.id === "soggetto").percentage =
        (d.data.soggetto / d.data.length) * 100
      legendData.find((d) => d.id === "misto").percentage =
        (d.data.misto / d.data.length) * 100
      legendData.find((d) => d.id === "dubbio").percentage =
        (d.data.dubbio / d.data.length) * 100
      ...
      for (let iii = data_misto.children.length - 1; iii > 0; iii--) {
        ...
        const item = {
          ...
          color: d3
            .color(color("misto"))
            .darker(0.3 * (+data_misto.children[iii].name - 1)),
    ...
    leaf_misto = leaf_misto.data(
      root_misto.leaves(),
      (l) => d.data.id + "-" + l.data.name
    )
    leaf_misto.exit().remove()
    leaf_misto = leaf_misto
      .enter()
      .append("g")
    ...
    leaf_misto_rect = leaf_misto_rect
      .enter()
      .append("rect")
      ...
      .attr("fill", (d) =>
        d3.color(color("misto")).darker(0.3 * (+d.data.name - 1))
      )

    leaf_misto.transition().delay(500).duration(250).style("opacity", 1)
    ...
    if (data_soggetto) {
      ...
      d3.select(".treemap-soggetto").attr(
        "transform",
        `translate(${x_soggetto + margin.left}, ${y_soggetto + margin.top})`
      )
      ...
      for (let iii = data_soggetto.children.length - 1; iii > 0; iii--) {
        const item = {
          ...
          color: d3
            .color(color("soggetto"))
            .darker(0.3 * (+data_soggetto.children[iii].name - 1)),
    ...
    leaf_soggetto = leaf_soggetto.data(
      root_soggetto.leaves(),
      (l) => d.data.id + "-" + l.data.name
    )
    leaf_soggetto.exit().remove()
    leaf_soggetto = leaf_soggetto
      .enter()
      .append("g")
    ...
    leaf_soggetto_rect = leaf_soggetto_rect
      .enter()
      .append("rect")
      ...
      .attr("height", (d) => d.y1 - d.y0)
      .attr("fill", (d) =>
        d3.color(color("soggetto")).darker(0.3 * (+d.data.name - 1))
      )

    leaf_soggetto.transition().delay(500).duration(250).style("opacity", 1)
  }
  selector.attr(
    "transform",
    `translate(${
      x.bandwidth() * x.paddingOuter() * (1 + xPadding) + x.bandwidth() / 2
    }, ${height + 41})`
  )
  selector.selectAll("*").remove()

  let selector_prev = selector
    .append("g")
    ...
    .on("click", () => {
      ...
      let to_be_selected = d3
        .selectAll(".serie-dubbio rect")
        .filter((d) => d.data.id === the_value)
        .data()[0]
  ...
  let selector_background = selector
    .append("rect")
    ...
    .call(
      d3
        ...
        .on("end", function () {
          ...
          let the_value = x.domain()[the_index]
          let the_selection = d3
            .selectAll(".serie-dubbio rect")
            .filter((d) => d.data.id === the_value)

          if (
            !the_selection.classed("selected") &&
            !the_selection.classed("filtered")
          ) {
            preSelection(the_selection.data()[0])
            selection(
              the_selection.data()[0],
              the_selection.classed("selected")
            )
          }
        })
  ...
  let selector_handle_area = selector_handle
    .append("rect")
    .attr("width", 36)
    .attr("x", -36 / 2)
    .attr("height", 36)
  ...
  let selector_handle_cursor = selector_handle
    .append("rect")
    .attr("width", x.step() * 2)
    .attr("x", (-x.step() * 2) / 2)
    .attr("height", 36)
  ...
  let selector_handle_line = selector_handle
    .append("line")
    .attr("x1", 0)
    .attr("y1", 0)
    .attr("x2", 0)
