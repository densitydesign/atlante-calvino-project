...
class VClass {
  initialize = () => {
    ...
    svg.selectAll("g.magazines > g").on("click", highlightFlux);
    svg.selectAll("g.collections > g").on("click", highlightFlux);
    svg.selectAll("g.collections-labels > g").on("click", highlightFlux);

    function highlightFlux() {
      d3.selectAll('g.stories > g').style('opacity', 0.2);
      d3.selectAll('g.magazines > g').style('opacity', 0.2);
      d3.selectAll('g.collections > g').style('opacity', 0.2);
      d3.selectAll('g.collections-labels > g').style('opacity', 0.2);
